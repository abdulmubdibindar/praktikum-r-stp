...
```{r mod06-create-crosstab-presentation}
library(gtsummary)
library(flextable)
...
```{r mod06-create-crosstab-calculation}
...
```{r mod06-display-chi-square-values}
...
```{r mod06-install-rcompanion, eval=FALSE}
install.packages("rcompanion")
```

```{r mod06-use-rcompanion}
library(rcompanion)
...
```{r mod06-install-rcpa3, eval=FALSE}
install.packages("RCPA3")
```

```{r mod06-calculate-lambda}
library(RCPA3)
...
```{r mod06-install-vcdextra, eval=FALSE}
install.packages("vcdExtra")
```

```{r mod06-calculate-gamma}
library(vcdExtra)
...
```{r mod06-display-gamma-components}
