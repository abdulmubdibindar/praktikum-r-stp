...
```{r mod01-import-first-sheet, echo=TRUE, R.options=list(max.print=50)}
...
```{r mod01-import-ubl-sheet, echo=TRUE, R.options=list(max.print=50)}
...
```{r mod01-glimpse-uinril, echo=TRUE, R.options=list(max.print=50)}
...
```{r mod01-check-data-types, echo=TRUE, R.options=list(max.print=80)}
...
```{r mod01-remove-columns-uinril, echo=TRUE, R.options=list(max.print=80)}
...
```{r mod01-remove-columns-ubl, echo=TRUE, R.options=list(max.print=80)}
...
```{r mod01-bind-rows-success, echo=TRUE, R.options=list(max.print=80)}
# Menggabungkan kedua data frame menjadi satu
...
```{r mod01-glimpse-final, echo=TRUE, R.options=list(max.print=80)}
...
2. **Identifikasi kolom yang bisa digabungkan**
   - Gunakan `glimpse()` untuk melihat nama dan tipe data setiap kolom
   - Keenam kolom berikut akan kita gunakan: kampus, jenis kelamin, kepemilikan mobil, motor, sepeda, dan jarak

3. **Seragamkan nama kolom**
   - Buat vektor `kolom_final` berisi nama standar untuk semua kolom. Anda dapat menyalin susunan berikut untuk membuat vektor tersebut:
   ```r
   kolom_final <- c("kampus", "jenis.kelamin", "kepemilikan.mobil", "kepemilikan.motor",
                    "kepemilikan.sepeda", "jarak")
   ```
   - Gunakan vektor tersebut untuk mengganti nama kolom di kedua dataset `colnames(data_itera_selected) <- kolom_final`

   Catatan: urutan 3 dan 4 dalam latihan ini sebenarnya bisa ditukar, sehingga tidak ada masalah kalau pun berbeda dengan urutan yang kalian pelajari sebelumnya

4. **Seleksi kolom yang sesuai** menggunakan `select()`
   - Pilih kolom-kolom yang sebelumnya kita sudah seleksi. Penting: urutan kolom yang diseleksi harus sesuai dengan urutan nama di `kolom_final`
   - Ingat menggunakan backtick (`) untuk kolom dengan karakter khusus

5. **Ganti nama kolom untuk kedua dataset** menggunakan `colnames()`
   - Ganti nama kolom untuk kedua dataset dengan `colnames(dataset_tiap_kampus) <- kolom_final`

6. **Gabungkan dengan `bind_rows()`**
