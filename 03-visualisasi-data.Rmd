...
# Mengubah nama variabel dan membersihkan missing values terlebih dahulu
data_ubl_clean <- data_ubl |>
  rename(biaya_dalam_sepekan = biaya_ribu) |>
  rename(jumlah_perjalanan_senin = jml_trip_senin) |>
  rename(jarak = jarak_km) |>
  drop_na()
...
fakultas_levels <- data_ubl_clean |> 
  count(fakultas) |> 
  pull(fakultas)

prodi_levels <- data_ubl_clean |> 
  count(prodi) |> 
  pull(prodi)

tingkat <- data_ubl_clean |> 
  count(tingkat_semester) |> 
  pull(tingkat_semester)

uang_saku_levels <- data_ubl_clean |> 
  count(uang_saku) |> 
  pull(uang_saku)

tingkat_semester_levels <- data_ubl_clean |> 
  count(tingkat_semester) |> 
  pull(tingkat_semester)

# Menerapkan baris unik tersebut menjadi factor ke dataset kita
data_ubl <- data_ubl_clean |>
  mutate(
    jenis_kelamin = factor(jenis_kelamin, levels = jk),
...
Kita akan mengubah kategori `factor` pada variabel `tingkat_semester` dan `uang_saku` agar lebih mudah dibaca saat divisualisasikan. Kita akan menggunakan fungsi `fct_recode()` dan `fct_relevel()` dari `forcats` (bagian dari `tidyverse`).

Berbeda sedikit dengan saat kita menggunakan `rename(Nama Baru = Nama Lama)` sebelumnya, struktur formasi kode yang dimiliki oleh `fct_recode()` menerima argumen sebagai berikut: `fct_recode(.f, "Level Baru" = "Level Lama")` di mana `.f` adalah nama variabel (kolom) yang nilai-nilainya akan kita terjemahkan.

```{r clean-data}
# Membersihkan dan mengubah nama level pada variabel tingkat_semester dan uang_saku
data_ubl_cleaned <- data_ubl %>%
  mutate(
    tingkat_semester = fct_recode(tingkat_semester,
...
diagram_batang <- ggplot(data_ubl_cleaned) +
  geom_bar(
    mapping = aes(x = tingkat_semester), fill = "skyblue",
    color = "black"
  ) +
  labs(
...
Kita bisa menambahkan variabel lain, misalnya `uang_saku`, ke dalam `aes()` dengan properti `fill` untuk membuat diagram batang bertumpuk. Ini menunjukkan proporsi uang saku di setiap tingkat semester.
...
::: rmdexercise
## Aktivitas Mandiri 1: Membuat dan Menginterpretasikan Diagram Batang [STP-3.2, STP-3.3].{capaian} {.unnumbered}
...
boxplot <- ggplot(data_ubl_cleaned) +
  geom_boxplot(mapping = aes(x = moda, y = jarak, fill = moda)) +
...
-   **MAPPING**: `mapping = aes(x = moda, y = jarak, fill = moda)` memetakan tiga hal: variabel kategori `moda` ke sumbu x, variabel numerik `jarak` ke sumbu y, dan `moda` ke warna isian `fill`.
...
-   **MAPPING**: `mapping = aes(x = umur, y = jarak)` memetakan dua variabel, `umur` ke sumbu x dan `jarak` ke sumbu y. Setiap baris data akan menjadi satu titik pada plot.
...
::: rmdexercise
## Aktivitas Mandiri 2: Histogram dan Boxplot untuk Variabel Numerik [STP-3.2, STP-3.3].{capaian} {.unnumbered}
...
2.  **Buat boxplot** yang membandingkan `jarak` berdasarkan `tingkat_semester` dan interpretasikan:
...
::: rmdexercise
## Aktivitas Mandiri 3: Visualisasi Komprehensif untuk `biaya_dalam_sepekan` [STP-3.1, STP-3.2, STP-3.3, STP-3.4].{capaian} {.unnumbered}
...
2.  Buat **boxplot** untuk `biaya_dalam_sepekan` berdasarkan `moda`
...
5.  **Apakah scatter plot cocok untuk `jenis_kelamin` vs `umur`? Mengapa tidak?**
