...
```{r mod08-install-broom, echo=TRUE, results='hide'}
...
```{r mod08-inspect-kampus-layanan-online, results='hide'}
...
Nilai konstanta berarti bahwa **dengan tinggal di kampus ($x=0$), mahasiswa masih akan mengeluarkan biaya sebesar `[beta_0]` $\times 1000$ rupiah** (ingat bahwa variabel `biaya sepekan` menggunakan satuan ribuan rupiah) untuk mengakses kampus selama sepekan. Kita bisa pahami hal tersebut **sebagai perkiraan biaya perjalanan mahasiswa yang tinggal di asrama**, misalnya.

Sementara itu, nilai koefisien sebesar `[beta_1]` berarti bahwa **penambahan jarak sebesar 1 km akan mengurangi biaya transportasi sepekan sebesar `[beta_1]`$\times 1000$ rupiah**. Artinya, makin jauh 1 km seorang mahasiswa tinggal dari kampus, makin kecil pula biaya transportasi sepekannya sebesar `[beta_1]`$\times 1000$ rupiah.
...
Kita juga dapat menggunakan penamaan variabel lain selain `x` dan `y` untuk memudahkan interpretasi. Untuk kasus ini, kita bisa menggunakan kata $biaya$ untuk menggantikan $y$, $jarak$ untuk menggantikan $x_1$, $lingKos_{sendiri}$ untuk menggantikan $x_2$, dan seterusnya.
...
Untuk persamaan linear berganda, kita tidak hanya mempertimbangkan pengaruh satu variabel independen seperti halnya pada regresi linear sederhana. Dengan adanya variabel independen lain, maka kita bisa menghitung pengaruh variabel independen lain tersebut.

Dalam kasus ini, penambahan jarak sebanyak 1 km akan meningkatkan biaya perjalanan sepekan mahasiswa sebesar `[beta_1]` $\times 1000$ rupiah, dengan asumsi `[ling]` tetap. Sementara itu, mahasiswa yang tinggal di `[ling]` akan meningkatkan biaya perjalanan sepekan mahasiswa sebesar `[beta_2]` $\times 1000$ rupiah, dengan asumsi `[jarak]` tetap.
