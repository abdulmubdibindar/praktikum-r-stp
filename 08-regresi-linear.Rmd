...
scp <- ggplot(
  ...
  geom_point(
    ...
    alpha = 0.01, # mengatur transparansi titik
...
Tuliskan interpretasi kalian terhadap perbedaan nilai koefisiennya.
...
scp <- ggplot(
  ...
  geom_point(
    ...
    alpha = 0.5, # mengatur transparansi
    size = 1.5, # mengatur ukuran titik
    shape = 15
  ) + # mengatur bentuk titik menjadi persegi
  labs(
...
```{r mod08-display-model-summary, echo=TRUE, results='hide'}
...
# Membuat nilai koefisien model
koef <- coef(model)

# Menampilkan koefisien variabel model
koef
```
...
Dalam keluaran perintah `coef()` kita dapat mengidentifikasi nilai $\beta_0$ atau konstanta melalui nilai di bawah `(Intercept)` dan nilai $\beta_1$ atau koefisien melalui nilai di bawah `x`. Ini sama saja dengan nilai-nilai di bawah kolom `Estimate` pada keluaran perintah `summary()` atau nilai-nilai di bawah kolom **Beta** pada keluaran `tbl_regression()`.

Berdasarkan keluaran-keluaran perintah tersebut, kita dapat mengetahui bagaimana persamaan regresi linearnya dengan mengganti bentuk umum persamaan regresi linear dengan angka-angka yang dihasilkan di `model`.
...
Tuliskan persamaan model tersebut dengan perintah `cat()` seperti berikut dan gantilah `[beta_0]` dan `[beta_1]` menjadi angka koefisien dan konstanta yang tepat.
...
scp <- ggplot(
  ...
  geom_abline(aes(
    intercept = koef[1], # ingat bahwa ini memanggil elemen pertama dari vektor 'koef'
    slope = koef[2],
    color = "red"
  )) +
  labs(
