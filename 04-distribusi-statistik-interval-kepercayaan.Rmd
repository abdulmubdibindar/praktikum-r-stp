...
```{r mod04-visualize-sampling-distribution}
...
```{r mod04-calculate-standard-error}
...
```{r mod04-estimate-se-from-sample}
...
```{r mod04-install-mkinfer, eval=FALSE}
install.packages("MKinfer")
```

Kemudian muat paket `MKinfer` tersebut.

```{r mod04-load-mkinfer}
library(MKinfer)
```

### Mengambil Satu Sampel

```{r mod04-sample-mean-sd}
...
```{r mod04-confidence-interval-mean}
...
```{r mod04-confidence-interval-proportion}
...
```{r mod04-question-ci-proportion}
