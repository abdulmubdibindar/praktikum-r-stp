# Script untuk membersihkan dataset UINRIL
# Mengikuti metodologi dari 02-statistik-deskriptif.Rmd

library(tidyverse)

cat("=== Membersihkan Dataset UINRIL ===\n\n")

# 1. Backup file asli dan import dataset
cat("1. Membackup file asli dan mengimpor dataset...\n")
original_file <- "datasets/DataUtama_mhsUINRIL.csv"
backup_file <- "datasets/_DataUtama_mhsUINRIL.csv"
...
data_uinril <- read.csv(backup_file, sep = ";", stringsAsFactors = FALSE)
cat(
    "   Dimensi awal:", nrow(data_uinril), "baris x",
    ncol(data_uinril), "kolom\n"
)

#  2. Cek missing values sebelum pembersihan
cat("\n2. Mengecek missing values...\n")
mv_before <- colSums(is.na(data_uinril))
...
# Normalisasi nama kolom dengan make.names
colnames(data_uinril) <- make.names(colnames(data_uinril))
...
for (col in numeric_cols) {
    if (col %in% colnames(data_uinril)) {
        na_count <- sum(is.na(data_uinril[[col]]))
        if (na_count > 0) {
            mean_val <- mean(data_uinril[[col]], na.rm = TRUE)
            data_uinril[[col]][is.na(data_uinril[[col]])] <- mean_val
...
for (col in categorical_cols) {
    if (col %in% colnames(data_uinril)) {
        na_count <- sum(is.na(data_uinril[[col]]) | data_uinril[[col]] == "")
        if (na_count > 0) {
            mode_val <- get_mode(data_uinril[[col]][data_uinril[[col]] != ""])
            data_uinril[[col]][is.na(data_uinril[[col]]) | data_uinril[[col]] == ""] <- mode_val
...
jk <- sort(unique(data_uinril$Jenis.Kelamin))
fakultas <- sort(unique(data_uinril$Fakultas))
prodi <- sort(unique(data_uinril$Prodi))
...
mv_after <- sum(is.na(data_uinril))
cat("   Total missing values setelah imputasi:", mv_after, "\n")
cat("   Dimensi akhir:", nrow(data_uinril), "baris x", ncol(data_uinril), "kolom\n")
...
cat("\n=== Pembersihan Dataset UINRIL Selesai ===\n")
