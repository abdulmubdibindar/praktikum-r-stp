# Script untuk membersihkan dataset ITERA
# Mengikuti metodologi dari 02-statistik-deskriptif.Rmd

library(tidyverse)

cat("=== Membersihkan Dataset ITERA ===\n\n")

# 1. Backup file asli dan import dataset
cat("1. Membackup file asli dan mengimpor dataset...\n")
original_file <- "datasets/DataUtama_mhsITERA.csv"
backup_file <- "datasets/_DataUtama_mhsITERA.csv"
...
data.itera <- read.csv(backup_file, sep = ";", stringsAsFactors = FALSE)
cat("   Dimensi awal:", nrow(data.itera), "baris x", ncol(data.itera), "kolom\n")

# 2. Cek missing values sebelum pembersihan
cat("\n2. Mengecek missing values...\n")
mv_before <- colSums(is.na(data.itera))
...
# Identifikasi kolom numerik dan kategoris (exclude Timestamp dan Kampus_PT)
numeric_cols <- c(
...
for (col in numeric_cols) {
    if (col %in% colnames(data.itera)) {
        na_count <- sum(is.na(data.itera[[col]]))
        if (na_count > 0) {
            mean_val <- mean(data.itera[[col]], na.rm = TRUE)
            data.itera[[col]][is.na(data.itera[[col]])] <- mean_val
...
for (col in categorical_cols) {
    if (col %in% colnames(data.itera)) {
        na_count <- sum(is.na(data.itera[[col]]) | data.itera[[col]] == "")
        if (na_count > 0) {
            mode_val <- get_mode(data.itera[[col]][data.itera[[col]] != ""])
            data.itera[[col]][is.na(data.itera[[col]]) | data.itera[[col]] == ""] <- mode_val
...
jk <- sort(unique(data.itera$Jenis.Kelamin))
fakultas <- sort(unique(data.itera$Fakultas))
prodi <- sort(unique(data.itera$Prodi))
...
mv_after <- sum(is.na(data.itera))
cat("   Total missing values setelah imputasi:", mv_after, "\n")
cat("   Dimensi akhir:", nrow(data.itera), "baris x", ncol(data.itera), "kolom\n")
...
cat("\n=== Pembersihan Dataset ITERA Selesai ===\n")
